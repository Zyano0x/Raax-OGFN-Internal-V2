...
void Drawing::BeginBatchedLines() {
    g_BatchedLines = true;
}
void Drawing::EndBatchedLines() {
    for (const auto& Line : g_BatchedLinesList) {
        if (Line.Outlined)
            g_DrawList->AddLine(ImVec2(Line.A.X, Line.A.Y), ImVec2(Line.B.X, Line.B.Y), ImColor(Line.OutlineColor.R, Line.OutlineColor.G, Line.OutlineColor.B, Line.OutlineColor.A), Line.OutlineThickness + Line.Thickness);
    }

    for (const auto& Line : g_BatchedLinesList) {
        g_DrawList->AddLine(ImVec2(Line.A.X, Line.A.Y), ImVec2(Line.B.X, Line.B.Y), ImColor(Line.Color.R, Line.Color.G, Line.Color.B, Line.Color.A), Line.Thickness);
    }
...
{
    if (ScreenPositionA.X != -1.f && ScreenPositionA.Y != -1.f &&
        ScreenPositionB.X != -1.f && ScreenPositionB.Y != -1.f) {
        if (g_BatchedLines)
            g_BatchedLinesList.push_back({ ScreenPositionA, ScreenPositionB, RenderColor, Thickness, Outlined, OutlineThickness, OutlineColor });
        else {
            if (Outlined)
                g_DrawList->AddLine(ImVec2(ScreenPositionA.X, ScreenPositionA.Y), ImVec2(ScreenPositionB.X, ScreenPositionB.Y), ImColor(OutlineColor.R, OutlineColor.G, OutlineColor.B, OutlineColor.A), OutlineThickness + Thickness);
...
void Drawing::Rect(const SDK::FVector2D& ScreenPosition, const SDK::FVector2D& ScreenSize, const SDK::FLinearColor& RenderColor,
...
void Drawing::CorneredRect(const SDK::FVector2D& ScreenPosition, const SDK::FVector2D& ScreenSize, const SDK::FLinearColor& RenderColor,
    float Thickness, bool Outlined, float OutlineThickness, const SDK::FLinearColor& OutlineColor) {
    if (ScreenPosition.X != -1.f && ScreenPosition.Y != -1.f) {
        ...
        auto DrawCorner = [&](const ImVec2& Corner, const ImVec2& Start, const ImVec2& End, ImU32 ColorU32, float LineThickness)
            {
        ...
        if (Outlined)
            DrawCorneredBox(ImColor(OutlineColor.R, OutlineColor.G, OutlineColor.B, OutlineColor.A), Thickness + OutlineThickness);

        DrawCorneredBox(ImColor(RenderColor.R, RenderColor.G, RenderColor.B, RenderColor.A), Thickness);
    }
}
void Drawing::Rect3D(const SDK::FVector2D(&BoxCorners)[8], const SDK::FLinearColor& RenderColor,
    float Thickness, bool Outlined, float OutlineThickness, const SDK::FLinearColor& OutlineColor) {
